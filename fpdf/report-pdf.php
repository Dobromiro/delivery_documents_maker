...
$MODULES_TOTAL = "SELECT SUM(`MODULES-QTY`) AS 'TOTAL_MODULES' FROM temp";


//$sql = "SELECT MODEL, DELIVERY_NO,  COUNT(*) AS 'BOXQTY' FROM temp GROUP BY MODEL ";
$sql = "SELECT MODEL, DELIVERY_NO, SUM(`MODULES-QTY`) AS 'MODULES', COUNT(*) AS 'BOXQTY' FROM temp GROUP BY MODEL";
$result = $conn->query($sql);
	$result2 = $conn->query($TOTAL);
	$result2->num_rows ;
	$row2 = $result2->fetch_assoc();
	$result3 = $conn->query($MODULES_TOTAL);
	$result3->num_rows ;
...
$pdf->Ln(10);
	...
	$pdf->Cell(55, 5, 'BOX QTY', 0, 0);	
	$pdf->Cell(55, 5, 'MODULES QTY', 0, 0);	
	$pdf->Ln();//Line break	
    while($row = $result->fetch_assoc()) {
		$pdf->Cell(55, 5, "$row[MODEL]", 0, 0);
		$pdf->Cell(55, 5, ": $row[BOXQTY]", 0, 0);
		$pdf->Cell(55, 5, ": $row[MODULES]", 0, 0);
		$pdf->Ln();//Line break	
    }	
	$pdf->Cell(55, 5, '  ', 0, 0);
	$pdf->Ln();//Line break
	...
	$pdf->Cell(55, 5, ": $row3[TOTAL_MODULES]", 0, 0);
	$pdf->Ln();//Line break
	/*

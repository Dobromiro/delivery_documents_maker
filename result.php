	...
	if (isset($_POST['POSITION']))
	{
		...
		try 
		{
			...
			else
			{
					...
					$sql = "SELECT COL1, COL2, COL3, COL4 FROM 'table 9'";
					$result = $conn->query($sql);

					if ($result->num_rows > 0) {
						// output data of each row
						while($row = $result->fetch_assoc()) {
							echo "<br> id: ". $row["COL1"]. " - Name: ". $row["COL2"]. " " . $row["COL3"] . "<br>";
						}
					} else {
						echo "0 results";
					}
					...
					/*if ($polaczenie->query("INSERT INTO microphones VALUES ('$MIC_NO', '$POSITION', '$BAT_LEV', '$CONDITION', '$MANAG_PERS', '$COMMENT', '$DATE', '$CHARGER' )"))
					{
